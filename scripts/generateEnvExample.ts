...
const lines: string[] = [
  ...
  "",
  "# Optional — winston logging. LOG_DIR is where rotated log files are",
  "# written (created on startup if missing); LOG_LEVEL controls console/file",
  "# verbosity (e.g. error, warn, info, debug). Defaults shown below.",
  `LOG_DIR=${example("LOG_DIR", shape.LOG_DIR)}`,
  `LOG_LEVEL=${example("LOG_LEVEL", shape.LOG_LEVEL)}`,
  "",
