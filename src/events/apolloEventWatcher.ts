...
async function tryHandleApolloMessage(client: BotClient, message: Message | PartialMessage): Promise<void> {
  // Deliberately independent of settings.apolloEventChannelId — the whole
  // point of this debug switch is to see a message's raw shape *before* the
  // channel/settings are configured correctly, so it can't be gated behind
  // the very configuration it's meant to help verify. Every message from
  // Apollo's own bot account, in any channel, gets logged while this is on
  // — not filtered to "has an embed", since that's exactly what's in
  // question when this is being used to debug a detection problem.
  if (LOG_APOLLO_EMBEDS && message.author?.id === APOLLO_BOT_USER_ID) {
    logger.info(
  ...
  // Filters on Apollo's actual Discord bot user id rather than a "some bot
  // posted a message" heuristic (`message.author?.bot`) — more precise, and
  // sidesteps whatever a verified app's author/webhook flags do or don't do
  // that made the heuristic unreliable in practice.
  if (message.author?.id !== APOLLO_BOT_USER_ID) return;
