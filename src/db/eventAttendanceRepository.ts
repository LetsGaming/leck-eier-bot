...
function rowToSignup(row: SignupRow): EventSignup {
  return {
    id: row.id,
    eventId: row.event_id,
    rawName: row.raw_name,
    normalizedName: row.normalized_name,
    choice: row.choice as RsvpChoice,
    userId: row.user_id,
...
function rowToVoiceLog(row: VoiceLogRow): EventVoiceLogRow {
  return {
    id: row.id,
    eventId: row.event_id,
    userId: row.user_id,
    action: row.action as EventVoiceLogRow["action"],
    at: row.at,
  };
}
 ...
 * hasn't been withdrawn. Only ever true for historical, fuzzy-matched rows
 * from the (now removed) Apollo scraper — a button-driven signup always
 * carries a real `user_id` from the interaction and is never `unmatched`/
 * `ambiguous`. Shared verbatim between `countUnmatchedSignupsStmt` (the
 * Overview badge), `listEventsWithUnresolvedSignups` (the "problems" list),
 * and `summarizeSignupsForEvents`'s `unresolved` aggregate, so the three can
...
const countUnmatchedSignupsStmt = db.prepare<[], { total: number }>(
  `SELECT COUNT(*) AS total FROM event_signups WHERE ${UNRESOLVED_SIGNUP_PREDICATE}`,
);
...
const selectDueScheduledEventsStmt = db.prepare<[string], EventRow>(
  `SELECT ${EVENT_COLUMNS} FROM events WHERE status = 'scheduled' AND starts_at <= ?`,
);
const selectDueActiveEventsStmt = db.prepare<[string], EventRow>(
  `SELECT ${EVENT_COLUMNS} FROM events WHERE status = 'active' AND ends_at <= ?`,
);
...
const updateEventFieldsStmt = db.prepare<{
  ...
  `UPDATE events SET title = @title, description = @description, starts_at = @startsAt, ends_at = @endsAt, updated_at = @updatedAt WHERE id = @id`,
);
const setEventActiveStmt = db.prepare<{ id: number; voiceChannelId: string | null; activatedAt: string; updatedAt: string }>(
  `UPDATE events SET status = 'active', voice_channel_id = @voiceChannelId, activated_at = @activatedAt, updated_at = @updatedAt WHERE id = @id`,
);
const setEventCompletedStmt = db.prepare<{ id: number; completedAt: string; updatedAt: string }>(
  `UPDATE events SET status = 'completed', completed_at = @completedAt, updated_at = @updatedAt WHERE id = @id`,
);
const setEventCancelledStmt = db.prepare<{ id: number; updatedAt: string }>(
  `UPDATE events SET status = 'cancelled', updated_at = @updatedAt WHERE id = @id`,
);
const markTrackingIncompleteStmt = db.prepare<{ id: number; updatedAt: string }>(
  `UPDATE events SET tracking_incomplete = 1, updated_at = @updatedAt WHERE id = @id`,
);
...
/**
 * Upserts a signup by real Discord user id — the write path for RSVP button
 * clicks. `normalized_name` is synthesized as `discord:<userId>` rather than
 * derived from a display name (there's nothing to fuzzy-match anymore), which
 * conveniently still satisfies the pre-existing `UNIQUE(event_id,
 * normalized_name)` table constraint, so this can use a plain `ON CONFLICT`
 * upsert without needing that constraint replaced — see migration v36's doc
...
const selectVoiceLogStmt = db.prepare<[number], VoiceLogRow>(
  `SELECT ${VOICE_LOG_COLUMNS} FROM event_voice_log WHERE event_id = ? ORDER BY at ASC, id ASC`,
);
const selectVoiceLogForUserStmt = db.prepare<[number, string], VoiceLogRow>(
  `SELECT ${VOICE_LOG_COLUMNS} FROM event_voice_log WHERE event_id = ? AND user_id = ? ORDER BY at ASC, id ASC`,
);
const insertVoiceLogStmt = db.prepare<{ eventId: number; userId: string; action: string; at: string }>(
  `INSERT INTO event_voice_log (event_id, user_id, action, at) VALUES (@eventId, @userId, @action, @at)`,
);
...
export function getEventByMessageId(messageId: string): Event | null {
...
export interface UpdateEventFieldsInput {
  title: string;
  description: string;
  startsAt: string;
  endsAt: string;
...
/** Edits a still-`scheduled` event's title/description/time — callers must check `status === 'scheduled'` themselves (this never freezes/rejects on its own, unlike the old Apollo-reparse upsert). */
export function updateEventFields(id: number, input: UpdateEventFieldsInput): Event {
  updateEventFieldsStmt.run({
    id,
    title: input.title,
    description: input.description,
    startsAt: input.startsAt,
...
/** Signups across every event still needing a manual member link — surfaced on Overview as an attention count (see docs/EVENT_ATTENDANCE.md). Only ever nonzero for historical, pre-migration signups. */
 ...
 * intended for a month-scoped dashboard view. Served by `idx_events_starts_at`.
 * Returns bare events, not hydrated with signups: list views use
 * `summarizeSignupsForEvents` aggregates instead of loading every signup row
 * per event (N+1).
 */
export function listEventsInRange({ fromIso, toIso, query }: EventsInRangeQuery): Event[] {
 ...
 */
export function listEventsAllMonths({ query, limit }: EventsAllMonthsQuery): Event[] {
  const likeQuery = `%${(query ?? "").trim().toLowerCase()}%`;
 ...
 */
export function listEventsWithUnresolvedSignups({ query, limit }: EventsAllMonthsQuery): Event[] {
  const likeQuery = `%${(query ?? "").trim().toLowerCase()}%`;
...
/** 'scheduled' events whose start time has passed — sweepEvents() activates these. */
export function listDueScheduledEvents(nowIso: string): Event[] {
  return selectDueScheduledEventsStmt.all(nowIso).map(rowToEvent);
}

/** 'active' events whose end time has passed — sweepEvents() finalizes/completes these. */
export function listDueActiveEvents(nowIso: string): Event[] {
...
export function listDueReminders(nowPlusLeadIso: string): Event[] {
  return selectDueRemindersStmt.all(nowPlusLeadIso).map(rowToEvent);
}
...
export function listSignups(eventId: number): EventSignup[] {
  return selectSignupsByEventStmt.all(eventId).map(rowToSignup);
}

export function getSignup(id: number): EventSignup | null {
  const row = selectSignupByIdStmt.get(id);
  return row ? rowToSignup(row) : null;
}

/** Manual reconciliation from the dashboard — only meaningful for historical, fuzzy-matched signups. `userId: null` unlinks (back to 'unmatched'); a non-null value always sets `matchSource: 'manual'`. */
...
/** The only function that writes attendance fields — never touches signup intent. Called from `finalizeAttendance()`/`recomputeAttendanceForEvent()` in `services/eventAttendance.ts`. */
...
export function listSignupsForUser(userId: string): (EventSignup & { eventTitle: string; eventStartsAt: string })[] {
...
export function listVoiceLog(eventId: number): EventVoiceLogRow[] {
  return selectVoiceLogStmt.all(eventId).map(rowToVoiceLog);
}

export function listVoiceLogForUser(eventId: number, userId: string): EventVoiceLogRow[] {
...
export interface VoiceLogEntryInput {
  eventId: number;
  userId: string;
  action: EventVoiceLogRow["action"];
  at: string;
}
