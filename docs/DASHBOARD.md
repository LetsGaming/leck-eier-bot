...
| `/events` | Event-Anwesenheit — every Apollo event the bot has detected, as cards grouped by month (prev/next, "Heute", a calendar-style month picker highlighting/badging months with data, title search), each card linking to its detail view. See [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md). |
| `/events/:eventId` | Event-Anwesenheit detail — one event's full signup table, each member's sign-up choice vs. actual attendance (on time/late/no-show/left early), a name/outcome filter, and a picker to manually link a name Apollo listed that couldn't be auto-matched to a member. See [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md). |
