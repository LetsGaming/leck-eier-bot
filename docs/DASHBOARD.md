...
| `/events` | Event-Anwesenheit — every event, as cards grouped by month (prev/next, "Heute", a calendar-style month picker highlighting/badging months with data, title search), each card linking to its detail view. See [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md). |
...
| `/event-templates` | Event-Vorlagen — create/edit/delete reusable event templates and publish an event from one. See [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md). |
