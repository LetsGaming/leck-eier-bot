...
`admin` is resolved from the **role's** Administrator flag, not a personal grant — if `@everyone` (or any other broadly-held role) is ever given Administrator, every member holding it silently becomes a dashboard `admin` on their next login. This mirrors Discord's own permission model (Administrator on a guild already implies full control of it), but it's worth keeping `@everyone` free of Administrator specifically so "who can log into the dashboard" stays a deliberate, reviewable list.
...
The role is stored on the session and checked per-route by `requireRole(...roles)` (`src/web/session.ts`); `requireAdmin` is the blanket `requireRole("bot-owner", "guild-owner", "admin")` most routes use, so **most features are open to all three tiers**. `PATCH /api/commands/:name` is the one exception today: it's narrowed to `requireRole("bot-owner", "guild-owner")` (route-level `preHandler`, applied on top of the blanket `requireAdmin`) because it can loosen a command's own `permissionGate` — including down to "everyone" — and an `admin` session shouldn't be able to grant other members access to a gated command. Other routes can be narrowed the same way later (e.g. for something destructive) without a schema or login-flow change. A role is fixed for the life of a session — a Discord-side ownership/role change takes effect on the next login, not live.
