...
| `moderator` | Anyone holding the configured `dashboardModeratorRoleId` role (Settings → Registrierung). Lowest tier — read-only by default (see "Feature access control" below). Off (nobody resolves to it) unless that role is set. |

`admin` is resolved from the **role's** Administrator flag, not a personal grant — if `@everyone` (or any other broadly-held role) is ever given Administrator, every member holding it silently becomes a dashboard `admin` on their next login. This mirrors Discord's own permission model (Administrator on a guild already implies full control of it), but it's worth keeping `@everyone` free of Administrator specifically so "who can log into the dashboard" stays a deliberate, reviewable list. The bot owner sees a warning banner on the Übersicht page if this is currently the case.

Anyone who resolves to none of the four gets a 403 and no session is created — there's no separate "logged in but no role" state to worry about downstream, since every session that exists already passed this check and carries its role.

The role is stored on the session but **re-verified against the live member cache on every `/api/*` request** (`createRequireDashboardUser` in `src/web/accessControl.ts`), not just trusted for the life of the session — a role grant, revoke, or guild departure takes effect on the very next request, not just at the next login (this only falls back to the session's original snapshot while the member cache is still warming up right after a restart). Requests are then checked per-route by `requireRole(...roles)` (`src/web/session.ts`, a fixed tier floor — e.g. `PATCH /api/commands/:name` requires `bot-owner`/`guild-owner`, since it can loosen a command's own `permissionGate` down to "everyone") or `requireFeature(key)` (`src/web/accessControl.ts`, a dashboard-admin-configurable gate — see below). GET routes have neither and are open to any of the four tiers.

### Feature access control

Beyond the fixed tier floors above, every gated dashboard *write* feature (registrations, events, event templates, reaction roles, birthdays, settings, command permissions) can be independently reconfigured from Settings → Zugriff (bot-owner/guild-owner only — managing these overrides is never itself delegable to `admin`/`moderator`) — the same `PermissionGate` shape (`{mode: "everyone"|"tier"|"role"}`) already used for Discord slash-command permissions, reused for the dashboard's own routes. `GET /api/access-control` lists each feature's default and current override; `PATCH /api/access-control/:key` sets or clears (`gate: null`) one. Every default reproduces the pre-existing behavior (admin-tier-or-higher, or guild-owner-or-higher for `commands.write`) until an override is actually set.

### Audit log

Every non-GET `/api/*` request by an authenticated session is recorded (actor, method, route pattern, status code, timestamp) in the `dashboard_audit_log` table and surfaced at `/audit-log` — bot-owner only. This is the accountability mechanism for a trust model where `admin`/`moderator` can be a broader group than individually-vetted-by-the-bot-owner staff: it records *who did what*, not *what changed*.
...
| `/events` | Event — two tabs (matching Settings' tab pattern). **Anwesenheit**: every event as cards grouped by month (prev/next, "Heute", a calendar-style month picker highlighting/badging months with data, title search), each card linking to its detail view; a "Neues Event" button picks a template and publishes from it. **Vorlagen**: create/edit/delete reusable event templates and publish an event from one. See [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md). |
| `/events/:eventId` | Event-Anwesenheit detail — one event's full signup table, each member's RSVP choice vs. actual attendance (on time/late/no-show/left early), a name/outcome filter, and (for a historical row) a picker to manually link a name that couldn't be auto-matched to a member. See [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md). |
| `/settings` | The leave-notification toggle, the [global font](#the-global-font), the register-gate/registration-tier roles and the "rules accepted" detection method, the default event/event-voice channels for event attendance tracking, and current session info (including your dashboard role). |
