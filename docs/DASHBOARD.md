...
| `/events` | Event-Anwesenheit — every Apollo event the bot has detected, as cards grouped by month (prev/next, "Heute", month picker, title search), each card linking to its detail view. See [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md). |
