...
### `events` / `event_signups` / `event_voice_log` / `event_templates`

Backs [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md) — see that doc for the full creation/tracking design. `settings` also gains two columns here: `apollo_event_channel_id` (kept as the DB column name from before the rename — fallback channel for a newly-published event; exposed in code/API as `defaultEventChannelId`) and `event_voice_channel_id` (fallback voice channel for attendance tracking).

`events` — one row per natively-created event, keyed by `message_id`. `status` walks `scheduled -> active -> completed` (or `-> cancelled` if the message is deleted, or the dashboard cancels it, while still `scheduled`). `starts_at`/`ends_at` freeze once `status` leaves `scheduled` — an edit mid-event can't move an in-flight measurement's goalposts. `voice_channel_id` snapshots the template's (or the global fallback) voice channel at activation. `tracking_incomplete` flags an event the bot was offline for some/all of. `reminded_at` guards the pre-start reminder ping against double-firing. `starts_at` also has a dedicated index (`idx_events_starts_at`) for the month-range queries the dashboard's list view uses. Renamed from `apollo_events` in migration v36, which also dropped the `apollo_event_id` column — see that migration's comment for why.

`event_signups` — one row per signed-up member, with two column groups written by two independent code paths that must never touch each other's columns: **intent** (`raw_name`, `normalized_name`, `choice`, `user_id`, `match_source`, `withdrawn_at`) is written once by `upsertSignupByUser()` on every RSVP button click; **attendance** (`attendance_status`, `first_joined_at`, `last_left_at`) is written only by `finalizeAttendance()`/`recomputeAttendanceForEvent()` in `src/services/eventAttendance.ts`, derived from the voice log. `match_source` is `'button'` for every native signup (a real user id from the interaction); `'auto'`/`'manual'`/`'unmatched'`/`'ambiguous'` only occur on historical, pre-migration rows from the removed Apollo-embed scraper.

`event_voice_log` — an append-only log of every join/leave (plus `present_at_start`/`present_at_end` snapshot rows) in the tracked voice channel while an event is active, for every non-bot member who touches the channel (not just signed-up ones, so a signup made after the fact can still reconstruct real attendance). This is the source of truth; `attendance_status` above is a cache derived from replaying it — see `deriveAttendance()`.
