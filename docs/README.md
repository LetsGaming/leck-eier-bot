...
| [EVENT_ATTENDANCE.md](EVENT_ATTENDANCE.md) | Event templates, creation/publishing, RSVP buttons, the scheduled/active/completed tracking state machine, and what on-time/late/no-show/left-early mean |
...
- **First-time setup** — [CONFIGURATION.md](CONFIGURATION.md) for what to put in `.env`, [PERMISSIONS.md](PERMISSIONS.md) for inviting the bot with the right permissions, then [DEPLOYMENT.md](DEPLOYMENT.md) for how to actually run it.
- **A feature isn't working after an update (nickname not changing, thread not created, role not assigned)** — [PERMISSIONS.md](PERMISSIONS.md) covers the permission/intent each feature needs and how to grant a newly-required one to a bot already in your server.
