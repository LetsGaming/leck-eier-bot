...
1. **Templates** (`Event-Vorlagen` in the nav) — a template has a name, a title/description with `{token}` placeholders, and optional defaults (channel, mention role, attendance voice channel). Title/description are rendered through the same shared token engine as birthdays/reaction-roles/registration (`src/shared/messageTemplate.ts`) — see `src/services/events.ts`'s `renderEventText()`. `{start_time}`/`{end_time}` are always available (resolve to Discord timestamp tokens, `<t:epoch:F>`); any other `{token}` in the template is a placeholder the publisher fills in.
2. **Publishing** — from a template's "Veröffentlichen" action on the dashboard, or `/event create` (Discord picks the template via autocomplete, then a modal collects placeholders + start/end time — capped at 3 custom placeholders per modal, since Discord allows at most 5 text inputs per modal and two are always start/end; a template with more has to be published from the dashboard, which has no such cap). Either path calls `publishEvent()`, which renders the text, posts the message, and creates the `events` row.
...
**Event-Vorlagen** in the nav manages templates (create/edit/delete/publish) — see "Creating and publishing an event" above.
